...
page_init(void)
{
	...
	pg_boot_alloc = PGNUM(PADDR(boot_alloc(0)));
	for (i = npages_basemem; i < pg_boot_alloc; i++) {
		pages[i].pp_ref = 0;
		pages[i].pp_link = NULL;
	}
	for (i = pg_boot_alloc; i < npages; i++) {
		pages[i].pp_ref = 0;
		pages[i].pp_link = page_free_list;
		page_free_list = &pages[i];
	}
...
mmio_map_region(physaddr_t pa, size_t size)
{
	...
	//
	// Your code here:
	physaddr_t upper = ROUNDUP(base + size, PGSIZE);
	if (upper > MMIOLIM)
