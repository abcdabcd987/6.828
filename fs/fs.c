...
file_block_walk(struct File *f, uint32_t filebno, uint32_t **ppdiskbno, bool alloc)
{
	...
	if (filebno < NDIRECT) {
		...
		*ppdiskbno = &f->f_direct[filebno];
		...
		// indirect block
		if (!f->f_indirect) {
			// indirect block list doesn't exists
		...
		// indirect block list must exist now
		uint32_t *indirects = (uint32_t*)diskaddr(f->f_indirect);
...
file_get_block(struct File *f, uint32_t filebno, char **blk)
{
	...
	if ((r = file_block_walk(f, filebno, &pblockno, 1)) < 0) {
		cprintf("file_get_block: file_block_walk %e", r);
		return r;
	}
	if (!*pblockno) {
		if ((*pblockno = alloc_block()) < 0)
			return -E_NO_DISK;
	}
